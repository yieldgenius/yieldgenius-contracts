 ...
 */
contract StrategyStellaswapMultiRewardsLP is StratFeeManager, GasThrottler {
     ...
     * @param _nativeToLp0Route ouput to lp0 route
     * @param _nativeToLp1Route output to lp1 route
     */
    constructor(
        address _want,
        uint256 _poolId,
        ...
        require(
            _nativeToLp0Route[0] == native,
            "nativeToLp0Route[0] != native"
        );
        require(
            _nativeToLp0Route[_nativeToLp0Route.length - 1] == lpToken0,
            "nativeToLp0Route[last] != lpToken0"
        );
        nativeToLp0Route = _nativeToLp0Route;

        lpToken1 = IUniswapV2Pair(want).token1();
        require(
            _nativeToLp1Route[0] == native,
            "nativeToLp1Route[0] != native"
        );
        require(
            _nativeToLp1Route[_nativeToLp1Route.length - 1] == lpToken1,
            "nativeToLp1Route[last] != lpToken1"
        );
    ...
    function _convertRewards() internal {
        ...
        if (rewardToNativeRoute.length != 0) {
            for (uint i; i < rewardToNativeRoute.length; i++) {
                uint256 toNative = IERC20(rewardToNativeRoute[i][0]).balanceOf(
                    address(this)
                );
                if (toNative > 0) {
                    IUniswapRouterETH(unirouter).swapExactTokensForTokens(
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 nativeFeesOwed = (IERC20(native).balanceOf(address(this)) *
            fees.total) / DIVISOR;
        ...
        uint256 strategistFeeAmount = (nativeFeesOwed * fees.strategist) /
            DIVISOR;
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, , , ) = IMasterChefV8(chef).userInfo(
            poolId,
            address(this)
        );
        return _amount;
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        ) = IMasterChefV8(chef).pendingTokens(poolId, address(this));
    ...
    function _giveAllowances() internal {
        ...
        if (rewardToNativeRoute.length != 0) {
            for (uint i; i < rewardToNativeRoute.length; i++) {
                ...
                IERC20(rewardToNativeRoute[i][0]).safeApprove(
                    unirouter,
                    type(uint).max
                );
            }
