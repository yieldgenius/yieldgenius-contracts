...
contract StrategyGLP is StratFeeManager {
    ...
    function rewardsAvailable() public view returns (uint256) {
        uint256 rewardGLP = IGMXTracker(glpRewardStorage).claimable(
            address(this)
        );
        uint256 rewardGMX = IGMXTracker(gmxRewardStorage).claimable(
            address(this)
        );
    ...
    function retireStrat() external {
        ...
        IBeefyVault.StratCandidate memory candidate = IBeefyVault(vault)
            .stratCandidate();
